    ...
    $.fn.awesomecomplete = function(options)
    {
        ...
        {   
            ...
            var $list = $('<ul/>').addClass(config.suggestionListClass)
                                  ...
                                  .hide()
                                  .css('width', $attachTo.outerWidth());
            $this.data('awesomecomplete-list', $list);
            $(window).resize(function() { $list.css('width', $attachTo.outerWidth());});
            ...
            {
                // tab = 9; enter = 13; up = 38; down = 40; esc = 27
		...
		if ((event.which == 13) && ($list.children('li.' + config.activeItemClass).length > 0))
		{
            ...
            $this.focus(function()
            {
                if ($list.children(':not(.' + config.noResultsClass + ')').length > 0) {
                    $list.css('width', $attachTo.outerWidth());
                    $list.show();
                }
            });
        });
...
// private helpers
    var processInput = function($this)
    {
        var term = $this.val();
        if (term.length > 1) {
            $this.parent().find('.arrow-down').show();
    ...
    var processData = function($this, data, term)
    {
        ...
        results.forEach(function(item, i, enumerable) {
...
// default functions
    ...
    $.fn.awesomecomplete.defaults = {
        ...
        wordDelimiter: /[^\da-z|ę|ó|ą|ś|ł|ż|ź|ń]+/ig
    };
})(jQuery);
